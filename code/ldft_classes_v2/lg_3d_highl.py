...
class LG3dAOHighl(LdftModel):
    ...
    works with four species instead of one, namely the species of the
    ...
    y-direction and one for the z-direction). The free energy functional
    is the one for the four species. It differs from the free energy
    functional of the AO-model just by a correction term accounting for
    the zero- and one-body interaction of the polymers. If one wants the
    free energy of the lattice gas, one would have to calculate the
    semi-grand potential of the previous free energy, where the polymer
    clusters are treated grand-canonically and the colloids canonically.
    In this class extra functions are supported for this. The colloids
    correspond to the species in the lattice gas.
    ...
    def __init__(self, size, epsi, mu_fix_c=False, mu_c=None,\
        ...
        super().__init__(size=size, mu_fix=[mu_fix_c, True, True, True],
                mu=[mu_c, mu_pc, mu_pc, mu_pc],
                dens=[dens_c, None, None, None],
                v_ext=[v_ext_c, v_ext_pc, v_ext_pc, v_ext_pc],
                r=r, r_hist=r_hist, err_hist=err_hist, it_hist=it_hist,
    ...
    @property
    def _dens_pc2(self):
        """The average density of the polymer species in y-direction
        (`float`, read-only).
        """
        return self._dens[2]
    ...
    @property
    def _dens_pc3(self):
        """The average density of the polymer species in z-direction
        (`float`, read-only).
        """
        return self._dens[3]
    ...
    def _v_ext_pc3(self):
        """The external potential acting on the polymer clusters in
        z-direction. (`np.array`, read-only)
        """
        return self._v_ext[3]
    ...
    @property
    def _r_pc3(self):
        """The density profile of the polymer species in z-direction.
        (`numpy.ndarray`, read-only)
        """
        return self._r[3]
    ...
    mu_pc3=_mu_pc3
    """The chemical potential of the polymer-cluster in z-direction
    (times the inverse temperature to make its dimension 1).
    (`float`, read-only)
    """
    ...
    dens_pc3=_dens_pc3
    """The average density of the polymer clusters in z-direction.
    (`float`, read-only)
    """
    ...
    r_pc3=_r_pc3
    """The density profile of the polymer clusters in z-direction.
    (`np.array`, read-only)
    """
    ...
    def _cal_n(self):
        ...
        n2 = self._boundary_roll(self._r_c, -1, axis=0) + self._r_pc1
        n3 = self._r_c + self._r_pc2
        n4 = self._boundary_roll(self._r_c, -1, axis=1) + self._r_pc2
        n5 = self._r_c + self._r_pc3
        n6 = self._boundary_roll(self._r_c, -1, axis=2) + self._r_pc3
        n7 = self._r_pc1
        n8 = self._r_pc2
        n9 = self._r_pc3
        n10 = self._r_c
    ...
    def _cal_Phi_ex_AO(self):
        ...
        Phi_ex = Phi0(n1)+Phi0(n2)+Phi0(n3)+Phi0(n4)+Phi0(n5)+Phi0(n6)\
                -Phi0(n7)-Phi0(n8)-Phi0(n9)-5*Phi0(n10)
        return Phi_ex
        
    def cal_F(self):
        """Calculates the free energy of the four component system. It
        ...
        F = (F_id + F_ex_AO
                ...
                *np.sum(-1+r_c+self._boundary_roll(r_c, -1, axis=1))
                - np.log(z_pc3+1)
                *np.sum(-1+r_c+self._boundary_roll(r_c, -1, axis=2)))
        return F
    ...
    def cal_mu_ex(self):
        ...
        mu_c_ex = np.log((1-n1)*(1-self._boundary_roll(n2, 1, axis=0))\
                *(1-n3)*(1-self._boundary_roll(n4, 1, axis=1))\
                *(1-n5)*(1-self._boundary_roll(n6, 1, axis=2))\
                /(1-n10)**5) + 6*np.log(z_pc+1)
        mu_pc1_ex = np.log((1-n1)*(1-n2)/(1-n7))
        mu_pc2_ex = np.log((1-n3)*(1-n4)/(1-n8))
        mu_pc3_ex = np.log((1-n5)*(1-n6)/(1-n9))
    ...
    def cal_bulk_mu_lg(cls, r_c, epsi):
        ...
        mu_pc = cls.translate_epsi_to_mu_pc(epsi)
        z_pc = np.exp(mu_pc)
        mu_c = (np.log(r_c) +6*cls._cal_dPhi_0(r_c+r_pc)
                -5*cls._cal_dPhi_0(r_c)-6*np.log(z_pc+1))
        return mu_c
    ...
    def cal_bulk_dmu_lg(cls, r_c, epsi):
        ...
        mu_pc = cls.translate_epsi_to_mu_pc(epsi)
        z_pc = np.exp(mu_pc)
        dmu = 1/r_c + 6*cls._cal_d2Phi_0(r_c+r_pc)*(1+dr_pc)\
                -5*cls._cal_d2Phi_0(r_c)
        return dmu
    ...
    def _cal_bulk_f_AO_id(cls, r_c, r_pc):
        ...
        """
        f_id = r_c*(np.log(r_c)-1) +3*r_pc*(np.log(r_pc)-1)
        return f_id
    ...
    def _cal_bulk_f_AO_ex(cls, r_c, r_pc):
        ...
        """
        f_ex = (6*cls._cal_Phi_0(r_c+r_pc)\
                -3*cls._cal_Phi_0(r_pc)\
                -5*cls._cal_Phi_0(r_c))
        return f_ex
    ...
    def cal_bulk_f_lg(cls, r_c, epsi):
        ...
        mu_pc = cls.translate_epsi_to_mu_pc(epsi)
        z_pc = np.exp(mu_pc)
        f_tilde = f_AO_id+f_AO_ex-3*np.log(z_pc+1)*(2*r_c-1)
        f_eff = f_tilde-3*r_pc*mu_pc
        return f_eff
    ...
    @classmethod
    def cal_bulk_coex_dens(cls, mu, epsi, initGuess=np.array([0.01,
            0.99])):
